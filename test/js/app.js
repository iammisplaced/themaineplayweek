...
import { SUPABASE_URL, SUPABASE_ANON_KEY, STORAGE_PREFIX } from "./env.js";
...
const STORAGE_KEY = `${STORAGE_PREFIX}showtimes-local-edit`;
const TMDB_KEY_STORAGE_KEY = `${STORAGE_PREFIX}tmdb-api-key-local`;
const THEME_STORAGE_KEY = `${STORAGE_PREFIX}tmp-theme`;
const VIEW_STORAGE_KEY = `${STORAGE_PREFIX}tmp-view`;
...
const THEATRE_GEO_CACHE_KEY = `${STORAGE_PREFIX}theatre-geocode-cache-v1`;
const THEATRE_GEO_CACHE_TTL_MS = 1000 * 60 * 60 * 24 * 90;
const DEFAULT_DAYS_VIEW_RADIUS_MILES = 50;
const LOCATION_PREFERENCE_STORAGE_KEY = `${STORAGE_PREFIX}tmp-location-preference-v1`;
const LOCATION_CHOOSER_SEEN_STORAGE_KEY = `${STORAGE_PREFIX}tmp-location-chooser-seen-v1`;
...
function bindEvents() {
  ...
  elements.results.addEventListener("click", (event) => {
    ...
    if (posterFilmPageUrl) {
      window.location.assign(posterFilmPageUrl);
      return;
    }
...
function buildTheatreGroupsFromFilmPagesSource(source) {
  ...
  for (const filmEntry of source.films) {
    ...
    for (const showing of showings) {
      ...
      if (!filmShowing) {
        filmShowing = { date: showDate, room: String(showing?.room || "").trim(), times: [], premiumTimes: [] };
...
function renderFilmOptions() {
  ...
  if (elements.showingRoomInput) {
    elements.showingRoomInput.disabled = !hasFilm;
  }
  if (elements.showingFestivalSelect) {
...
function importShowtimesCsv(data, csvContent) {
  ...
  for (let rowIndex = 1; rowIndex < rows.length; rowIndex += 1) {
    ...
    if (!filmMatches.length) {
      ...
      if (filmYear !== null) film.year = filmYear;
      if (filmTmdbId !== null) film.tmdbId = filmTmdbId;
      ...
      for (const show of (festivalShowsToRender || showsToRender)) {
        ...
        if (state.view === "theatres") {
          ...
          if (state.view === "festivals") {
            ...
            const roomValue =
              String(show?.room || "").trim() ||
              String(show?.roomByTheatre?.[theatreKey] || "").trim();
            meta.textContent = [show.city || "", roomValue ? `Room: ${roomValue}` : ""].filter(Boolean).join(" · ");
            if (showFestivalIds.length) {
              const ribbon = document.createElement("span");
              ribbon.className = "festival-entry-ribbon festival-entry-ribbon-inline";
              ribbon.textContent = festivalBadgeLabel;
    ...
    Object.values(rowsByFilm).forEach((row) => {
      ...
      if (view === "days") {
        ...
        dateKeys.forEach((date) => {
          ...
          if (!dayRow.roomByTheatre[theatreKey] && row.roomByDate[date]) {
            dayRow.roomByTheatre[theatreKey] = row.roomByDate[date];
          }
